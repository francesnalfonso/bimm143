...
We ran AlphaFold via GoogleColab

In particular we used their AlphaFold2_mmseqs2 version that uses mmseq2 rather than HMMMer for sequence search.

The main outputs include a set of PDB structure files along with matching JSON format files that tells us how goof the resulting models
...
results_pdb <- "HIVpr1dimer_23119/" 

# File names for all PDB models
pdb_files <- list.files(path=results_pdb,
                        pattern="*.pdb",
                        full.names = TRUE)
...
## Predicted Alignment for Domains

```{r}
library(jsonlite)

# Listing of all PAE JSON files
pae_files <- list.files(path=results_pdb,
                        pattern=".*model.*\\.json",
                        full.names = TRUE)
```
...
```{r}
aln_file <- list.files(path=results_pdb,
                       pattern=".a3m$",
                        full.names = TRUE)
aln_file
```
